...
class ImprovedFlexibleEntityDisambiguator(FlexibleEntityDisambiguator):
    """Improved entity disambiguator with name handling, works with any embedding model"""
    ...
    def extract_name_parts(self, descriptor: str) -> Dict[str, any]:
    ...
    def create_entity_embeddings(self, entities: List[Dict[str, str]]) -> Dict[str, any]:
        ...
        embeddings = {
            ...
            "normalized": self.encode(normalized_descriptors),
            "names": self.encode(full_names),
            "normalized_names": self.encode(normalized_names),
            "first_names": self.encode(first_names),
            "last_names": self.encode(last_names),
    ...
    def search(self, query: str, entities: List[Dict[str, str]], 
        ...
        # For full names and semantic queries, use hybrid approach
            ...
            # Semantic search
            query_embeddings = {
                "original": self.encode([query]).reshape(1, -1),
                "normalized": self.encode([query_normalized]).reshape(1, -1)
            }
...
if __name__ == "__main__":
    # Test the improved flexible disambiguator
    entities = [
    ...
    test_queries = ["John", "John Smith", "Jhon Smith", "Software Engineer"]
    
    print("="*80)
    print("TESTING IMPROVED FLEXIBLE ENTITY DISAMBIGUATOR")
    print("="*80)
    ...
    try:
        ...
        potion_embeddings = potion_improved.create_entity_embeddings(entities)
        
        for query in test_queries:
            results, search_time, match_type = potion_improved.search(
                query, entities, potion_embeddings, threshold=0.5
            )
            print(f"\nQuery: '{query}' - Found {len(results)} matches")
            for r in results[:3]:
                print(f"  - {r['descriptor'][:40]}... (score: {r['similarity']:.3f})")
    ...
    print("-"*50)
    try:
        minilm_improved = ImprovedFlexibleEntityDisambiguator(
            model_name="sentence-transformers/all-MiniLM-L6-v2",
            model_type="sentence-transformer"
        )
        minilm_embeddings = minilm_improved.create_entity_embeddings(entities)
        
        for query in test_queries:
            results, search_time, match_type = minilm_improved.search(
                query, entities, minilm_embeddings, threshold=0.5
            )
            print(f"\nQuery: '{query}' - Found {len(results)} matches")
            for r in results[:3]:
                print(f"  - {r['descriptor'][:40]}... (score: {r['similarity']:.3f})")
